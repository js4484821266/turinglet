...
Turinglet은 사용자의 메시지 내용뿐 아니라 타이핑 상태, 침묵 시간, 감정 강도, 선제 발화 쿨다운을 함께 보고 “언제, 몇 개의 메시지로, 어떤 톤으로 응답할지”를 조절하는 이벤트 기반 AI 대화 프로토타입이다.
...
- [`frontend/src/App.tsx`](frontend/src/App.tsx)의 입력 이벤트가 [`/api/chat/typing`](backend/src/app.ts)으로 전달되어, 사용자가 입력 중이면 서버가 응답을 미룬다.
...
| [`backend/src/app.ts`](backend/src/app.ts) | 코드 | REST API, Socket.IO emit, reactive/proactive 타이머, 인증 라우트를 묶는 핵심 파일이다. | High | 가장 먼저 읽어야 한다. |
...
| [`frontend/src/App.tsx`](frontend/src/App.tsx) | 코드 | 인증 화면, 채팅 화면, 관리자 화면을 모두 포함한다. | High | 프론트 흐름의 중심이다. |
...
- API, timer, socket, scheduler 결합: [`backend/src/app.ts`](backend/src/app.ts)
...
- DB 저장과 snapshot: [`backend/src/db/store.ts`](backend/src/db/store.ts), [`database/migrations/001_init.sql`](database/migrations/001_init.sql)
...
- 프론트 입력/스캔: [`AuthPanel`](frontend/src/App.tsx), [`BrowserQRCodeReader`](frontend/src/App.tsx)
...
- 사용자 입력: [`ChatPanel`](frontend/src/App.tsx)
- typing 전송 주기: [`onDraftChange`](frontend/src/App.tsx)가 입력마다 `true`를 보내고 4초 뒤 `false`를 보낸다.
...
- 반응 예약: [`scheduleReactivePlan`](backend/src/app.ts)
- 메시지 큐 실행: [`queuePlanMessages`](backend/src/app.ts)
- socket event 수신: [`socket.on('message')`](frontend/src/App.tsx), [`socket.on('presence')`](frontend/src/App.tsx)
...
- 관련 파일: [`frontend/src/App.tsx`](frontend/src/App.tsx), [`backend/src/app.ts`](backend/src/app.ts), [`backend/src/engine/orchestrator.ts`](backend/src/engine/orchestrator.ts), [`backend/src/adapters/mockProvider.ts`](backend/src/adapters/mockProvider.ts), [`backend/src/db/store.ts`](backend/src/db/store.ts)
...
- 주요 함수 또는 메서드: [`scheduleReactivePlan`](backend/src/app.ts), [`queuePlanMessages`](backend/src/app.ts), [`planForUserMessage`](backend/src/engine/orchestrator.ts), [`generateMultiMessagePlan`](backend/src/adapters/mockProvider.ts), [`appendMessage`](backend/src/db/store.ts)
- 주요 변수 또는 상수: [`reactivePlanTimers`](backend/src/app.ts), [`reactiveSequence`](backend/src/app.ts), [`userContinuationGraceMs`](backend/src/config.ts), [`reactiveResponseMaxWaitMs`](backend/src/config.ts)
...
- 관련 파일: [`frontend/src/App.tsx`](frontend/src/App.tsx), [`backend/src/app.ts`](backend/src/app.ts), [`backend/src/db/store.ts`](backend/src/db/store.ts), [`shared/src/index.ts`](shared/src/index.ts)
- 관련 리소스: [`database/migrations/001_init.sql`](database/migrations/001_init.sql)
- 주요 클래스: 확인 필요
- 주요 함수 또는 메서드: [`sendTyping`](frontend/src/App.tsx), [`onDraftChange`](frontend/src/App.tsx), [`setTypingPresence`](backend/src/db/store.ts), [`isUserTyping`](backend/src/db/store.ts)
- 주요 변수 또는 상수: frontend typing timer 4000ms, backend typing freshness 6000ms, [`TypingSchema`](backend/src/app.ts)
- 입력: `{ isTyping: boolean }`
...
- 관련 파일: [`backend/src/app.ts`](backend/src/app.ts), [`scheduler/src/index.ts`](scheduler/src/index.ts), [`backend/src/engine/messageGenerator.ts`](backend/src/engine/messageGenerator.ts), [`backend/src/engine/orchestrator.ts`](backend/src/engine/orchestrator.ts), [`backend/src/db/store.ts`](backend/src/db/store.ts)
...
- 주요 함수 또는 메서드: [`runProactiveLoop`](backend/src/app.ts), [`startScheduler`](backend/src/app.ts), [`evaluateProactiveDecision`](scheduler/src/index.ts), [`inferSilence`](backend/src/engine/messageGenerator.ts), [`planForSilence`](backend/src/engine/orchestrator.ts), [`recordProactiveEvent`](backend/src/db/store.ts)
...
- 관련 파일: [`frontend/src/App.tsx`](frontend/src/App.tsx), [`backend/src/app.ts`](backend/src/app.ts), [`backend/src/db/store.ts`](backend/src/db/store.ts)
- 관련 리소스: [`database/migrations/001_init.sql`](database/migrations/001_init.sql)
- 주요 클래스: 확인 필요
- 주요 함수 또는 메서드: [`AdminPanel`](frontend/src/App.tsx), [`loadOverview`](frontend/src/App.tsx), [`loadSessionMessages`](frontend/src/App.tsx), [`listUsers`](backend/src/db/store.ts), [`listSessions`](backend/src/db/store.ts), [`listMessagesForSession`](backend/src/db/store.ts), [`listProactiveEvents`](backend/src/db/store.ts)
- 주요 변수 또는 상수: [`AdminOverview`](frontend/src/App.tsx), [`AdminUserRow`](frontend/src/api.ts), [`AdminSessionRow`](frontend/src/api.ts), [`AdminProactiveEventRow`](frontend/src/api.ts)
...
| [`attachSocket`](backend/src/app.ts) | 함수 | [`backend/src/app.ts`](backend/src/app.ts) | Socket.IO 서버를 만들고 `join_session` room을 처리한다. | [`ChatPanel`](frontend/src/App.tsx) |
| [`scheduleReactivePlan`](backend/src/app.ts) | 함수 | [`backend/src/app.ts`](backend/src/app.ts) | 사용자 메시지 후 실제 assistant 응답을 지연 계획한다. | [`ConversationOrchestrator`](backend/src/engine/orchestrator.ts) |
| [`queuePlanMessages`](backend/src/app.ts) | 함수 | [`backend/src/app.ts`](backend/src/app.ts) | `MultiMessagePlan.messages`를 delay에 맞춰 저장/emit한다. | [`appendMessage`](backend/src/db/store.ts) |
| [`runProactiveLoop`](backend/src/app.ts) | 함수 | [`backend/src/app.ts`](backend/src/app.ts) | 활성 세션을 순회하며 선제 발화를 판단한다. | [`evaluateProactiveDecision`](scheduler/src/index.ts) |
| [`auth`](backend/src/app.ts) | 함수 | [`backend/src/app.ts`](backend/src/app.ts) | `x-session-id` header를 검증하고 session을 touch한다. | [`getSessionById`](backend/src/db/store.ts) |
...
| [`planForUserMessage`](backend/src/engine/orchestrator.ts) | 메서드 | [`backend/src/engine/orchestrator.ts`](backend/src/engine/orchestrator.ts) | typing/continuation이면 `sendCount: 0`, 아니면 provider plan을 반환한다. | [`scheduleReactivePlan`](backend/src/app.ts) |
| [`planForSilence`](backend/src/engine/orchestrator.ts) | 메서드 | [`backend/src/engine/orchestrator.ts`](backend/src/engine/orchestrator.ts) | 감정 강도에 따라 empathy 또는 check-in 메시지를 만든다. | [`runProactiveLoop`](backend/src/app.ts) |
...
| [`SqliteStore`](backend/src/db/store.ts) | 클래스 | [`backend/src/db/store.ts`](backend/src/db/store.ts) | SQLite 기반 `Store` 구현이다. | [`database/migrations/001_init.sql`](database/migrations/001_init.sql) |
| [`PostgresStore`](backend/src/db/store.ts) | 클래스 | [`backend/src/db/store.ts`](backend/src/db/store.ts) | PostgreSQL 기반 `Store` 구현이다. | [`createStore`](backend/src/db/index.ts) |
| [`Store`](backend/src/db/store.ts) | interface | [`backend/src/db/store.ts`](backend/src/db/store.ts) | 사용자/세션/메시지/typing/proactive/snapshot 메서드 계약이다. | [`SqliteStore`](backend/src/db/store.ts) |
| [`getConversationSnapshot`](backend/src/db/store.ts) | 메서드 | [`backend/src/db/store.ts`](backend/src/db/store.ts) | 최근 메시지 시간, 감정 강도, typing 상태를 `ConversationSnapshot`으로 합친다. | [`ConversationSnapshot`](shared/src/index.ts) |
| [`setTypingPresence`](backend/src/db/store.ts) | 메서드 | [`backend/src/db/store.ts`](backend/src/db/store.ts) | typing state를 insert/update한다. | [`isUserTyping`](backend/src/db/store.ts) |
| [`isUserTyping`](backend/src/db/store.ts) | 메서드 | [`backend/src/db/store.ts`](backend/src/db/store.ts) | 6000ms freshness 기준으로 typing 여부를 판단한다. | [`scheduleReactivePlan`](backend/src/app.ts) |
...
| [`decodeQrPayload`](backend/src/utils/qrPayload.ts) | 함수 | [`backend/src/utils/qrPayload.ts`](backend/src/utils/qrPayload.ts) | QR payload를 파싱하고 Zod schema로 검증한다. | [`LoginSchema`](backend/src/app.ts) |
...
| [`AuthPanel`](frontend/src/App.tsx) | component | [`frontend/src/App.tsx`](frontend/src/App.tsx) | QR 가입, QR 로그인, 카메라/이미지 QR 스캔을 처리한다. | [`api`](frontend/src/api.ts) |
| [`ChatPanel`](frontend/src/App.tsx) | component | [`frontend/src/App.tsx`](frontend/src/App.tsx) | 메시지 조회, socket 구독, typing 전송, 메시지 전송을 처리한다. | [`scheduleReactivePlan`](backend/src/app.ts) |
| [`AdminPanel`](frontend/src/App.tsx) | component | [`frontend/src/App.tsx`](frontend/src/App.tsx) | 관리자 overview와 세션 메시지를 표시한다. | [`/api/admin/overview`](backend/src/app.ts) |
| [`useAppStore`](frontend/src/store.ts) | hook/store | [`frontend/src/store.ts`](frontend/src/store.ts) | session, QR, messages, presence 상태를 저장한다. | [`ChatPanel`](frontend/src/App.tsx) |
...
- 관련 파일: [`backend/src/app.ts`](backend/src/app.ts), [`backend/src/engine/orchestrator.ts`](backend/src/engine/orchestrator.ts), [`backend/src/db/store.ts`](backend/src/db/store.ts)
- 관련 함수 또는 클래스: [`scheduleReactivePlan`](backend/src/app.ts), [`ConversationOrchestrator.planForUserMessage`](backend/src/engine/orchestrator.ts), [`isUserTyping`](backend/src/db/store.ts)
- 어떤 입력을 받는가: `sessionId`, `userText`, `ConversationSnapshot`, typing state
- 어떤 출력을 만드는가: `MultiMessagePlan` 또는 delayed retry
- 시간 또는 성능상 주의할 점: 각 session에 하나의 [`reactivePlanTimers`](backend/src/app.ts)만 유지하며, retry는 attempt 20회와 [`reactiveResponseMaxWaitMs`](backend/src/config.ts)로 제한된다.
...
- 관련 파일: [`scheduler/src/index.ts`](scheduler/src/index.ts), [`backend/src/app.ts`](backend/src/app.ts)
- 관련 함수 또는 클래스: [`evaluateProactiveDecision`](scheduler/src/index.ts), [`runProactiveLoop`](backend/src/app.ts)
...
- 시간 또는 성능상 주의할 점: [`startScheduler`](backend/src/app.ts)는 [`config.proactivePollMs`](backend/src/config.ts)마다 모든 active session을 순회한다. 세션 수가 많아지면 DB query 수가 증가한다.
...
- 관련 파일: [`backend/src/db/store.ts`](backend/src/db/store.ts), [`shared/src/index.ts`](shared/src/index.ts)
- 관련 함수 또는 클래스: [`getConversationSnapshot`](backend/src/db/store.ts), [`mapState`](backend/src/db/store.ts), [`ConversationSnapshot`](shared/src/index.ts)
...
| `VITE_BACKEND_ORIGIN` | [`frontend/src/api.ts`](frontend/src/api.ts), [`frontend/src/App.tsx`](frontend/src/App.tsx) | 프론트에서 backend origin override | 미설정 시 현재 host의 `:4000` | LAN/배포 환경에서 API 주소를 바꾼다. |
...
| typing debounce | [`frontend/src/App.tsx`](frontend/src/App.tsx) | 입력 멈춤 후 typing false 전송 시간 | `4000ms` | 너무 짧으면 사용자가 아직 입력 중인데 false가 될 수 있다. |
| typing freshness | [`backend/src/db/store.ts`](backend/src/db/store.ts) | typing true를 유효하게 보는 시간 | `6000ms` | 너무 길면 assistant 응답이 과하게 지연된다. |
...
실제 실행 결과: [`backend/tests/policy.test.ts`](backend/tests/policy.test.ts)의 3개 테스트는 통과했지만, [`backend/tests/auth-qr.test.ts`](backend/tests/auth-qr.test.ts)는 migration 파일을 `backend/database/migrations/001_init.sql`에서 찾으면서 실패했다. 즉, 현재 root `npm test`는 완료 성공이 아니다.
...
| QR 인증 | `npm test` 중 [`backend/tests/auth-qr.test.ts`](backend/tests/auth-qr.test.ts) | [`backend/src/app.ts`](backend/src/app.ts), [`database/migrations/001_init.sql`](database/migrations/001_init.sql) | Vitest 결과 | 현재 실패 확인. `process.cwd()`가 backend라 migration 경로가 잘못 계산된다. |
...
| QR 가입/로그인 수동 검증 | frontend에서 가입 후 QR payload로 로그인 | [`frontend/src/App.tsx`](frontend/src/App.tsx), [`backend/src/utils/qrPayload.ts`](backend/src/utils/qrPayload.ts) | `sessionId`, `userId`, 초기 assistant greeting | QR payload가 정상 생성/복원되는지 본다. |
| reactive 채팅 | 메시지 입력 후 assistant 메시지 수신 | [`ChatPanel`](frontend/src/App.tsx), [`scheduleReactivePlan`](backend/src/app.ts) | socket `message`, `presence` | 입력 직후 202가 오고, 지연 후 assistant 메시지가 socket으로 오는지 확인한다. |
| typing 중 개입 방지 | 긴 문장을 입력하며 typing 상태 유지 | [`sendTyping`](frontend/src/App.tsx), [`isUserTyping`](backend/src/db/store.ts) | assistant 응답 지연 | 사용자가 입력 중일 때 메시지가 발송되지 않아야 한다. |
| proactive 발화 | `PROACTIVE_MIN_SILENCE_MS`를 낮춰 긴 침묵 상황을 만들기 | [`runProactiveLoop`](backend/src/app.ts), [`evaluateProactiveDecision`](scheduler/src/index.ts) | assistant proactive message, `proactive_events` row | cooldown과 silence 조건이 동작하는지 본다. |
| 관리자 대시보드 | frontend에서 관리자 탭 클릭 | [`AdminPanel`](frontend/src/App.tsx), [`/api/admin/overview`](backend/src/app.ts) | 사용자/세션/이벤트 표 | DB에 쌓인 대화 상태를 관찰한다. 인증 없음에 주의. |
...
Turinglet은 단순히 사용자가 말하면 바로 답하는 챗봇이 아니라, 사용자의 타이핑 상태와 침묵 시간, 감정 강도, 선제 발화 쿨다운을 보고 응답 타이밍을 조절하는 AI 대화 프로토타입입니다. 프론트는 React와 Socket.IO로 실시간 채팅을 만들고, 백엔드는 Express에서 메시지를 저장한 뒤 즉시 응답하지 않고 별도 타이머로 반응 계획을 계산합니다. 핵심은 “무엇을 말할지”와 “언제 말할지”를 분리한 점입니다.

### 1분 설명

이 프로젝트는 대화형 AI에서 문장 생성보다 대화 리듬을 실험한 프로젝트입니다. 사용자가 메시지를 보내면 서버는 먼저 메시지를 DB에 저장하고 `202 Accepted`를 반환합니다. 그 다음 [`scheduleReactivePlan`](backend/src/app.ts)이 타이핑 상태와 문장 끝 패턴을 보고, 사용자가 더 말할 가능성이 있으면 응답을 미룹니다. 응답이 필요하다고 판단되면 [`ConversationOrchestrator`](backend/src/engine/orchestrator.ts)가 provider를 통해 `MultiMessagePlan`을 받고, 여러 개의 짧은 메시지를 delay와 presence 상태에 맞춰 Socket.IO로 전송합니다. 별도 [`scheduler`](scheduler/src/index.ts)는 긴 침묵과 쿨다운을 보고 선제 발화를 제한적으로 보냅니다. QR 기반 가입/로그인과 관리자 대시보드도 있어서 세션 복원과 실험 관찰까지 할 수 있습니다.

### 기술 질문 대응용 설명

구조는 npm workspace 모노레포입니다. [`shared/src/index.ts`](shared/src/index.ts)에 메시지, snapshot, provider interface 타입을 두고, [`backend`](backend)은 Express/Socket.IO 서버와 store/provider/orchestrator를 조립합니다. DB는 [`Store`](backend/src/db/store.ts) interface 뒤에 SQLite와 PostgreSQL 구현을 둔 형태입니다. 사용자의 입력은 [`frontend/src/App.tsx`](frontend/src/App.tsx)에서 REST API로 보내고, 서버의 assistant 메시지와 presence는 Socket.IO room `session:{sessionId}`로 받습니다. reactive 흐름에서는 [`ConversationOrchestrator.planForUserMessage`](backend/src/engine/orchestrator.ts)가 typing 또는 continuation이면 `sendCount: 0`으로 응답을 미루고, proactive 흐름에서는 [`evaluateProactiveDecision`](scheduler/src/index.ts)가 침묵 시간과 쿨다운을 판단합니다. LLM provider는 [`LLMProviderAdapter`](shared/src/index.ts)로 추상화되어 mock과 로컬 HF 서버를 바꿔 쓸 수 있습니다.

### AI 코딩 도구 사용 질문 대응

GitHub Copilot과 Codex를 활용해 구현 속도를 높였고, 구조 이해, 테스트, 디버깅, 결과 검증을 직접 수행했습니다. 특히 코드를 그대로 생성해 끝낸 것이 아니라, backend의 메시지 지연 흐름, scheduler의 선제 발화 조건, provider fallback, QR 인증과 DB 저장 구조를 직접 읽고 연결 관계를 확인했습니다. 테스트도 직접 실행해 proactive 정책 테스트는 통과하고 QR 인증 테스트는 migration 경로 문제로 실패한다는 점까지 확인했습니다.

## 15. 내가 반드시 이해해야 할 코드

| 우선순위 | 파일 또는 함수 | 왜 중요한가 | 읽을 때 확인할 것 |
| ---- | -------- | ------ | ---------- |
| 1 | [`backend/src/app.ts`](backend/src/app.ts) | 전체 서버 흐름, API, socket, reactive/proactive 타이머가 모두 모여 있다. | `scheduleReactivePlan`, `queuePlanMessages`, `runProactiveLoop`, auth flow |
...
| 5 | [`backend/src/db/store.ts`](backend/src/db/store.ts) | DB 저장과 snapshot 생성의 실제 구현이다. | `appendMessage`, `isUserTyping`, `getConversationSnapshot`, SQLite/Postgres 차이 |
| 6 | [`frontend/src/App.tsx`](frontend/src/App.tsx) | 사용자 입력, QR 인증, socket 수신, admin UI가 들어 있다. | `AuthPanel`, `ChatPanel`, `AdminPanel`, typing timer |
...
- [`backend/tests/auth-qr.test.ts`](backend/tests/auth-qr.test.ts)는 `process.cwd()`가 backend일 때 [`database/migrations/001_init.sql`](database/migrations/001_init.sql)을 찾지 못한다. 실제 `npm test`에서 실패했다.
- [`/api/admin/overview`](backend/src/app.ts)와 [`/api/admin/sessions/:sessionId/messages`](backend/src/app.ts)는 인증 없이 접근 가능하다. 프로토타입 관찰용이라면 괜찮지만 실제 서비스에는 위험하다.
...
- [`backend/src/app.ts`](backend/src/app.ts)의 timer 기반 plan은 메모리 안에만 있다. 서버 재시작 시 pending assistant message와 reactive plan은 사라진다.
- [`queuePlanMessages`](backend/src/app.ts)는 send 직전에 typing이면 message send를 skip한다. 그런데 proactive event는 queue 직후 기록되어 실제 발송 실패와 기록이 어긋날 수 있다.
- [`backend/src/adapters/hfLocalProvider.ts`](backend/src/adapters/hfLocalProvider.ts)는 `nextState`를 구체 enum으로 검증하지 않고 cast한다.
...
- [`frontend/src/App.tsx`](frontend/src/App.tsx)에 인증, 채팅, 관리자 UI가 한 파일에 몰려 있다. 기능이 늘면 유지보수 비용이 커진다.
...
| --- | ----- | ----- | ----- |
| Easy | QR 인증 테스트의 migration 경로 수정 | `npm test`가 실제로 통과한다. | [`backend/tests/auth-qr.test.ts`](backend/tests/auth-qr.test.ts), [`database/migrations/001_init.sql`](database/migrations/001_init.sql) |
...
| Medium | 관리자 API에 간단한 인증 추가 | 로컬 외 환경에서 대화 데이터 노출 위험을 줄인다. | [`backend/src/app.ts`](backend/src/app.ts), [`frontend/src/App.tsx`](frontend/src/App.tsx) |
| Medium | [`frontend/src/App.tsx`](frontend/src/App.tsx)를 `AuthPanel`, `ChatPanel`, `AdminPanel` 파일로 분리 | UI 유지보수성이 좋아진다. | [`frontend/src/App.tsx`](frontend/src/App.tsx), [`frontend/src/store.ts`](frontend/src/store.ts) |
| Medium | proactive event에 실제 sent/skip 결과 기록 | 관리자 대시보드의 관찰 정확도가 올라간다. | [`queuePlanMessages`](backend/src/app.ts), [`recordProactiveEvent`](backend/src/db/store.ts) |
