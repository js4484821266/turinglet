...
산마고우(Saammagou)는 사용자의 메시지 내용뿐 아니라 타이핑 상태, 침묵 시간, 감정 강도, 선제 발화 쿨다운을 함께 보고 “언제, 몇 개의 메시지로, 어떤 톤으로 응답할지”를 조절하는 이벤트 기반 AI 대화 프로토타입이다. 이름은 `컴패니언 → 대나무숲 → 죽마고우(竹馬故友) → 살아 있는 존재`라는 발상 흐름에서 유래했다.

제품 표시 이름만 산마고우(Saammagou)로 변경했으며, 기존 디렉터리명과 `@turinglet/*`, `turinglet-id` 같은 내부 패키지·프로토콜 식별자는 호환성을 위해 유지한다.
