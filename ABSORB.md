...
삼마고(Saammago)는 사용자의 메시지 내용뿐 아니라 타이핑 상태, 침묵 시간, 감정 강도, 선제 발화 쿨다운을 함께 보고 “언제, 몇 개의 메시지로, 어떤 톤으로 응답할지”를 조절하는 이벤트 기반 AI 대화 프로토타입이다. 이름은 `사람처럼 메시지를 보내는 AI → 무엇이든 받아주는 존재 → 대나무숲 → 竹 → 죽마고우 → 죽다의 반의어인 살다 → 산마고우 → 음소 동화 → 삼마고`라는 연상 과정에서 유래했다.
